...
def print_cpg_statistics(cpg_dict: dict) -> None:
    ...
    for _, value in statistics.items():
        ...
        cg_edges.append(value['num_of_cg_edges'])
...
def cpg_statistics(cpg_dict: dict) -> dict:
    ...
    for _, value in cpg_dict.items():
        for func_cgnode in value:
            ...
            # call graph
            callees = func_cgnode.callees
            for _, v in callees.items():
                for callee in v:
                    statistics[filename]['num_of_cg_edges'] += calculate_function_cg(callee, cpg_dict)

            statistics[filename]['num_of_edges'] += statistics[filename]['num_of_ast_edges'] + statistics[filename]['num_of_cfg_edges'] + statistics[filename]['num_of_dfg_edges'] + statistics[filename]['num_of_cg_edges']
